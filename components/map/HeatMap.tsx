...
import { interpolateColor, getInfrastructureIcon, getPriorityColor } from '@/lib/utils';
import type { HeatDataPoint, HeatZone, GreenInfrastructureRecommendation } from '@/types';
  ...
  return (
    <div className="relative h-full w-full">
      ...
      >
        ...
        {activeLayers.includes('heat') && heatData.map((point, index) => (
          <CircleMarker
            key={`heat-${index}`}
        ...
        {/* Heat Zones Polygons */}
        {activeLayers.includes('heat') && heatZones.map((zone) => (
          <Polygon
            key={zone.id}
            positions={zone.coordinates}
            pathOptions={{
              fillColor: zone.severity === 'extreme' ? '#7f1d1d' :
                         zone.severity === 'high' ? '#dc2626' :
                         zone.severity === 'moderate' ? '#f97316' : '#facc15',
              fillOpacity: 0.3,
              color: zone.severity === 'extreme' ? '#7f1d1d' :
                     zone.severity === 'high' ? '#dc2626' :
                     zone.severity === 'moderate' ? '#f97316' : '#facc15',
              weight: 2
            }}
          ...
          >
            <Popup>
              <div className="text-sm">
                ...
                <p>Severity: <span className="font-medium capitalize">{zone.severity}</span></p>
        ...
        {activeLayers.includes('recommendations') && recommendations.map((rec) => (
          <CircleMarker
            key={rec.id}
            center={rec.location}
            radius={8}
            ...
            <Popup>
              <div className="text-sm min-w-[200px]">
                ...
                <div className="space-y-1">
                  <p>Type: <span className="font-medium">{getInfrastructureIcon(rec.type)} {rec.type.replace('_', ' ')}</span></p>
                  <p>Priority: <span className="font-medium capitalize">{rec.priority}</span></p>
                  <p>Cooling Effect: <span className="font-medium text-emerald-600">-{rec.estimatedCoolingEffect.toFixed(1)}°C</span></p>
                  <p>Est. Cost: <span className="font-medium">${(rec.estimatedCost / 1000000).toFixed(1)}M</span></p>
                </div>
              </div>
            </Popup>
          </CircleMarker>
